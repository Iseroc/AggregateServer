 ...
 */
public class AggregateServerConsoleClient {

	public AggregateServerConsoleClient() {
	...
	public static void main(String[] args) throws Exception {
		...
		PropertyConfigurator.configureAndWatch(AggregateServerConsoleClient.class
				.getResource("log.properties").getFile(), 5000);
		AggregateServerConsoleClient sampleConsoleClient = new AggregateServerConsoleClient();
		try {
	 ...
	 * @return
	 */
	protected static String dateTimeToString(String title, DateTime timestamp,
	...
	protected ServerStatusListener serverStatusListener = new ASServerStatusListener();
	...
	protected SubscriptionAliveListener subscriptionAliveListener = new ASSubscriptionAliveListener();

	protected SubscriptionNotificationListener subscriptionListener = new ASSubscriptionNotificationListener();

	protected String userName;

	protected CertificateValidationListener validationListener = new ASCertificateValidationListener();
	 ...
	 */
	protected NodeId browse(NodeId nodeId, NodeId prevId)
					...
					try {
						...
						if (upReferences.size() == 1)
							...
							for (int i = 0; i < upReferences.size(); i++)
								printf("%d - %s\n", i,
										referenceToString(upReferences.get(i)));
			...
			case ACTION_TRANSLATE:
				...
				try {
					...
					for (BrowsePathTarget pathTarget : pathTargets) {
						...
						if (!pathTarget.getRemainingPathIndex().equals(
								UnsignedInteger.MAX_VALUE))
							targetStr = targetStr + " - RemainingPathIndex: "
	 ...
	 *
	 */
	protected void callMethod(NodeId nodeId, NodeId methodId)
		...
		if (value.getStatusCode().isNotBad()) {
			...
			if (value.isNull())
				...
				if (showReadValueDataType
						&& Attributes.Value.equals(attributeId))
					try {
	...
	protected EndpointDescription discoverEndpoints(
			ApplicationDescription serverApp) throws URISyntaxException {
		...
		if (discoveryUrls != null) {
			...
			println(String.format("%s - %-50s - %-20s - %-20s - %s", "#",
					"URI", "Security Mode", "Security Policy",
					"Transport Profile"));
			for (String url : discoveryUrls) {
				discoveryClient.setUri(url);
				try {
					for (EndpointDescription ed : discoveryClient
							.discoverEndpoints()) {
						println(String
								.format("%s - %-50s - %-20s - %-20s - %s",
										i++,
										ed.getEndpointUrl(),
										ed.getSecurityMode(),
					...
					println("Cannot discover Endpoints from URL " + url + ": "
							+ e.getMessage());
				}
			}
			System.out
	 ...
	 * @return
	 */
	protected String eventFieldsToString(QualifiedName[] fieldNames,
			Variant[] fieldValues) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < fieldValues.length; i++) {
			Object fieldValue = fieldValues[i] == null ? null : fieldValues[i]
					.getValue();
	...
	protected String eventToString(NodeId nodeId, QualifiedName[] fieldNames,
			Variant[] fieldValues) {
		return String.format("Node: %s Fields: %s", nodeId,
				eventFieldsToString(fieldNames, fieldValues));
	}
		...
		if (node instanceof AnalogItemType)
			try {
				...
				analogInfoStr = analogInfoStr
						+ (range == null ? "" : String.format(
								" Range=(%f; %f)", range.getLow(),
								range.getHigh()));
			} catch (Exception e) {
				printException(e);
			}
	...
	protected void initEventFieldNames() throws StatusException {
		if (eventFieldNames[eventFieldNames.length - 1] == null) {
			...
			int namespaceIndex;
			namespaceIndex = client.getNamespaceTable().getIndex(
					SAMPLE_ADDRESS_SPACE);
			eventFieldNames[eventFieldNames.length - 2] = new QualifiedName(
					namespaceIndex, "MyVariable");
			eventFieldNames[eventFieldNames.length - 1] = new QualifiedName(
					namespaceIndex, "MyProperty");
		}
	}
	 ...
	 * @throws ServerListException
	 */
	public void initialize(String[] args) throws URISyntaxException,
		...
		//client = new UaClient(defaultServerUri);
		//client = new UaClient("opc.tcp://130.233.110.96:4841");
	...
	public void mainMenu() throws ServerListException, URISyntaxException {
		...
		connect();
		println("mainMenu():n connect() kutsuttu");

		// Subscribe to items specified from command line
		subscribeToInitialItems();
		println("mainMenu():n subscribeToInitialItems() kutsuttu");
	...
	protected void parseSecurityMode(String arg) {
		...
		if (secModeStr == 'n')
			...
			throw new IllegalArgumentException(
					"parameter for SecuirtyMode (-s) is invalid, expected 'n', 's' or 'e'; was '"
							+ secModeStr + "'");
	}
	...
	protected String promptServerUri() throws IllegalArgumentException {
		...
		if (input.equals("y"))
			return defaultServerUri; // Using the default URI
	 ...
	 *
	 */
	public void read(NodeId nodeId) throws ServiceException, StatusException {
	...
	public DataValue readAndReturnDataValue(NodeId nodeId) throws ServiceException, StatusException {
		println("read node " + nodeId);
		UnsignedInteger attributeId = readAttributeId();
		DataValue value = client.readAttribute(nodeId, attributeId);
		println(dataValueToString(nodeId, attributeId, value));
	 ...
	 *
	 */
	protected void readHistory(NodeId nodeId) throws ServiceException,
		...
		if (node instanceof UaVariable) {
			...
			try {
				...
				case 2:
					...
					values = client.historyReadProcessed(nodeId, startTime,
							endTime, 60 * 1000.0, // your 5 min sample rate
							// in milliseconds
			...
			if (!object.getEventNotifier().contains(
					EventNotifierClass.HistoryRead)) {
			...
			try {
				...
				events = client.historyReadEvents(nodeId, startTime, endTime,
						UnsignedInteger.ZERO, eventFilter,
						TimestampsToReturn.Source);
				...
				if (events != null) {
					...
					for (int i = 0; i < events.length; i++)
						println("Event "
								+ (i + 1)
								+ " = "
								+ eventFieldsToString(eventFieldNames,
										events[i].getEventFields()));
				}
			} catch (Exception e) {
				printException(e);
			}
	...
	protected NodeId readMethodId(NodeId nodeId)
		...
		NodeClass nodeClass = client.getAddressSpace().getNode(nodeId)
				.getNodeClass();
	...
	public void relaySubscription(NodeId nodeId, UnsignedInteger attributeId) {
		try {
			// Create the subscription
			if (subscription == null)
				subscription = createSubscription();
	...
	protected void subscriptionMenu() throws ServiceException, StatusException {
		...
		do {
			...
			if (input.equals("r")) {
				...
				println("Current publishing interval is "
						+ subscription.getPublishingInterval() + " ms");
				...
				try {
					...
					try {
						...
						println("Publishing interval changed to " + interval
								+ " ms");
					} catch (NumberFormatException e) {
						printException(e);
					}
	 ...
	 * @throws ServiceException
	 */
	protected void write(NodeId nodeId) throws ServiceException,
	...
	ApplicationDescription discoverServer(String uri)
		...
		println(String.format("%s - %-25s - %-15s - %-30s - %s", "#", "Name",
				"Type", "Product", "Application"));
		for (int i = 0; i < serverList.size(); i++) {
			final ApplicationDescription s = serverList.get(i);
			println(String.format("%d - %-25s - %-15s - %-30s - %s", i, s
	...
	/*
	 * Below are helper functions used in relaying a subscription to an aggregating node to the underlying aggregated node
	 */
	
	public void storeInternalClient (AggregateServerConsoleClient intClient) {
		((ASMonitoredDataItemListener) dataChangeListener).storeInternalClient(intClient);
	}
	
	public void updateIdMap(TargetServer ts) {
		((ASMonitoredDataItemListener) dataChangeListener).updateIdMap(ts);
	}
	
	public void storeMonitoredIdPair(NodeId remoteId, NodeId aggId) {
		((ASMonitoredDataItemListener) dataChangeListener).storeMonitoredIdPair(remoteId, aggId);
	}
	
	public HashMap<NodeId, NodeId> getMonitoredPairs() {
		return ((ASMonitoredDataItemListener) dataChangeListener).getIdMap();
	}
	
	public void removeSubscription(NodeId remoteId, NodeId aggId) throws ServiceException, StatusException {
		synchronized (dataChangeListener) {
			...
			try {
				MonitoredItemBase removedItem = null;
					for (MonitoredItemBase item : subscription.getItems())
					{
						if (item.getNodeId().equals(remoteId)){
							UnsignedInteger handle = item.getClientHandle();
							removedItem = subscription.removeItem(handle);
							((ASMonitoredDataItemListener) dataChangeListener).removeMonitoredIdPair(remoteId);
							printf(removedItem != null ? "Item %s removed\n"
									: "No such item: %s\n", handle);
							break;
						}
				...
				subscription.setPublishingEnabled(true);
				MonitoredItemBase[] remainingItems = subscription.getItems();
				if (remainingItems.length >= 1) {
					System.out.println("At least one item left in subscription");
				}
				else {
					System.out.println("No items left in subscription");
				}
			}
		}
	}
