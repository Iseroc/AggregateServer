 ...
 */
public class ASUserValidator implements UserValidator {

	private static Logger logger = Logger.getLogger(ASUserValidator.class);
	 ...
	 */
	@Override
	public boolean onValidate(Session session, ServerUserIdentity userIdentity)
		...
		if (userIdentity.getType().equals(UserTokenType.UserName))
			if (userIdentity.getName().equals("opcua")
				...
				return true;
			else if (userIdentity.getName().equals("opcua2")
					&& userIdentity.getPassword().equals("opcua2"))
				return true;
			else
				return false;
	 ...
	 */
	@Override
	public void onValidationError(Session session, UserIdentityToken userToken,
			Exception exception) {
		logger.warn("User validation failed: userToken=" + userToken
				+ " error=" + exception);
	}
